...
function JustDroppedDeals() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchDealsByParams(`sortBy=Recent&pageSize=10&pageNumber=0`)
      .then((initialGames) => {
  ...
  const handleViewMore = async () => {
    ...
    try {
      const newGames = await fetchDealsByParams(`sortBy=Recent&pageSize=10&pageNumber=${nextPage}`);

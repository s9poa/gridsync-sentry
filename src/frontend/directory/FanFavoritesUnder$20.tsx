...
function FanFavoritesUnder$20() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchDealsByParams(`sortBy=Reviews&upperPrice=20&pageSize=10&pageNumber=0`)
      .then((initialGames) => {
  ...
  const handleViewMore = async () => {
    ...
    try {
      const newGames = await fetchDealsByParams(`sortBy=Reviews&upperPrice=20&pageSize=10&pageNumber=${nextPage}`);

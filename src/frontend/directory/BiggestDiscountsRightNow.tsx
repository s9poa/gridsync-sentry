...
function BiggestDiscountsRightNow() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const firstPage = await fetchDealsByParams(`sortBy=Savings&pageSize=${GAMES_PER_PAGE}&pageNumber=0`);
        setPagesData((prev) => {
  ...
  const goToPage = async (pageNum: number) => {
    ...
    try {
      ...
      const newPage = await fetchDealsByParams(`sortBy=Savings&pageSize=${GAMES_PER_PAGE}&pageNumber=${apiPageIndex}`);
      setPagesData((prev) => {
  ...
  return (
    <main className={`${styles.main} wrapper`}>
      <h1><Link to="/" aria-label="Go back to home page" className={styles.homeLink}><i className="fa-solid fa-chevron-left" aria-hidden="true"></i></Link>Biggest Discounts Right Now</h1>
...
export default BiggestDiscountsRightNow;

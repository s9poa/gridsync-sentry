...
function Under$10Steals() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const firstPage = await fetchDealsByParams(`upperPrice=10&pageSize=${GAMES_PER_PAGE}&pageNumber=0`);
        setPagesData((prev) => {
  ...
  const goToPage = async (pageNum: number) => {
    ...
    try {
      ...
      const newPage = await fetchDealsByParams(`upperPrice=10&pageSize=${GAMES_PER_PAGE}&pageNumber=${apiPageIndex}`);
      setPagesData((prev) => {
  ...
  return (
    <main className={`${styles.main} wrapper`}>
      <h1><Link to="/" aria-label="Go back to home page" className={styles.homeLink}><i className="fa-solid fa-chevron-left" aria-hidden="true"></i></Link>Under $10 Steals</h1>
